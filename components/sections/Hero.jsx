...
export default function Hero() {
  ...
  const [displayedCode, setDisplayedCode] = useState(Array(codeLines.length).fill(""));
  ...
  const formatCodeLine = (line) => {
    // Keywords
    line = line.replace(/\b(const|function|return|this)\b/g, '<span class="text-[#ff79c6] dark:text-[#ff79c6]">$1</span>');
    
    // Values
    line = line.replace(/\b(true|false)\b/g, '<span class="text-[#bd93f9] dark:text-[#bd93f9]">$1</span>');
    
    // Strings
    line = line.replace(/'([^']*)'/g, '<span class="text-[#f1fa8c] dark:text-[#f1fa8c]">\'$1\'</span>');
    
    // Operators
    line = line.replace(/(&amp;&amp;|>=)/g, '<span class="text-[#ff79c6] dark:text-[#ff79c6]">$1</span>');
    
    // Properties
    line = line.replace(/\b(name|skills|hardWorker|quickLearner|problemSolver|hireable|length)\b(?!['"])/g, 
        ...
        <div className="space-y-6 md:space-y-8 order-2 md:order-1">
          <div className="space-y-4">
            <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold">
              Hello,
              <br />
              This is <span className="bg-clip-text text-transparent bg-gradient-to-r from-[#3B82F6] to-[#6366F1] dark:from-[#F472B6] dark:to-[#EC4899]">QITONG LAN</span>,
              <br />
              I'm a Professional <span className="bg-clip-text text-transparent bg-gradient-to-r from-[#2563EB] to-[#4F46E5] dark:from-[#38BDF8] dark:to-[#818CF8]">Full-stack Engineer.</span>
            </h1>
          </div>
          ...
          <div className="flex space-x-4">
            <Link href="https://github.com/tonglam" target="_blank">
              <Button variant="outline" size="icon" className="rounded-full w-10 h-10 sm:w-12 sm:h-12 border-[#3B82F6] dark:border-[#F472B6] hover:bg-gradient-to-r hover:from-[#3B82F6]/20 hover:to-[#6366F1]/20 dark:hover:from-[#F472B6]/20 dark:hover:to-[#EC4899]/20 flex items-center justify-center">
                <GithubIcon className="text-[#3B82F6] dark:text-[#F472B6]" size={20} />
              </Button>
            </Link>
            
            <Link href="https://www.linkedin.com/in/qitonglan/" target="_blank">
              <Button variant="outline" size="icon" className="rounded-full w-10 h-10 sm:w-12 sm:h-12 border-[#3B82F6] dark:border-[#F472B6] hover:bg-gradient-to-r hover:from-[#3B82F6]/20 hover:to-[#6366F1]/20 dark:hover:from-[#F472B6]/20 dark:hover:to-[#EC4899]/20 flex items-center justify-center">
                <LinkedinIcon className="text-[#3B82F6] dark:text-[#F472B6]" size={20} />
              </Button>
            </Link>

            <Link href="https://x.com/tong_lam_14" target="_blank">
              <Button variant="outline" size="icon" className="rounded-full w-10 h-10 sm:w-12 sm:h-12 border-[#3B82F6] dark:border-[#F472B6] hover:bg-gradient-to-r hover:from-[#3B82F6]/20 hover:to-[#6366F1]/20 dark:hover:from-[#F472B6]/20 dark:hover:to-[#EC4899]/20 flex items-center justify-center">
                <TwitterIcon className="text-[#3B82F6] dark:text-[#F472B6]" size={20} />
              </Button>
            </Link>
          </div>
        ...
        >
          <div className="bg-white dark:bg-gradient-to-br dark:from-[#282a36] dark:to-[#1e1f29] rounded-lg shadow-xl dark:shadow-2xl overflow-hidden border border-gray-200 dark:border-gray-800 w-full md:w-[110%] md:-ml-5">
            <div className="flex items-center gap-2 p-2 md:p-4 bg-gray-100 dark:bg-gradient-to-r dark:from-[#1e1f29] dark:to-[#282a36] border-b border-gray-200 dark:border-gray-800 overflow-hidden">
              ...
              <div className="ml-1 text-xs md:text-base text-gray-500 dark:text-gray-400 truncate">developer.js</div>
            </div>
            <div className="p-3 sm:p-4 md:p-8 font-mono text-xs md:text-base overflow-x-auto">
              <pre className="text-gray-800 dark:text-white w-full">
                {displayedCode.map((line, index) => (
                  <div key={index} className="line text-xs md:text-base leading-tight md:leading-relaxed">
