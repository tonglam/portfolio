...
function formatBlogPost(page) {
  ...
  return {
    ...
    image: getPreferredImageUrl(page.properties),
...
/**
 * Get the preferred image URL, prioritizing R2ImageUrl when available
 */
function getPreferredImageUrl(properties) {
  // First check if we have an R2ImageUrl
  if (properties.R2ImageUrl?.url) {
    return properties.R2ImageUrl.url;
  }

  // Fall back to the original image URL
  return getImageUrl(properties.Image);
}
